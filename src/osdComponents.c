...
void newHorizon(void)
{
    ...
    x_c = halfX;
    y_c = halfY * (1 - pitch / theta_max);
    x_c2 = halfX + sinroll * flag;
    y_c2 = halfY * (1 - pitch / theta_max) + cosroll * flag;
    
    for (i = 10; i <= 90; i += 10) {
        d1 = sqrtf(17 * 17 + powf(i * M_PI / 180.0f / theta_max * halfY, 2));
        alpha1 = atan2f((i * M_PI / 180.0f / theta_max * halfY), 17);

        // d2 = sqrtf(50 * 50 + powf(i * M_PI / 180 / theta_max * halfY, 2));
        // alpha2 = atanf((i * M_PI / 180 / theta_max * halfY) / 50);

        // + i.e. down
        t_a[idx] = 3;
        a_a[idx] = i;
        x_a[idx] = halfX + d1 * cosf(alpha1 - roll);
        y_a[idx++] = (halfY) * (1 - pitch / theta_max) + d1 * sinf(alpha1 - roll);
        t_a[idx] = 3;
        a_a[idx] = i;
        x_a[idx] = halfX - d1 * cosf(alpha1 + roll);
        y_a[idx++] = (halfY) * (1 - pitch / theta_max) + d1 * sinf(alpha1 + roll);
    ...
    //centre mark
    osdDrawLine(halfX - 25, halfY - 5, halfX - 20, halfY, 1, 0);
    osdDrawLine(halfX - 25, halfY + 5, halfX - 20, halfY, 1, 0);
    osdDrawLine(halfX + 25, halfY - 5, halfX + 20, halfY, 1, 0);
    osdDrawLine(halfX + 25, halfY + 5, halfX + 20, halfY, 1, 0);
