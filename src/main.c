...
mainTask(void *unused)
{
    ...
    while (1) {
        ...
        for (i = 50; i < 210; i += 10) {
            ...
            if (!((alt - offset + 120 - i) % 20)
                && (alt - offset + 120 - i + 10 < alt || alt - offset + 120 - i - 10 > alt)) {
        ...
        if (osdData.PAL) {
            osdSetCursor(OSD_WIDTH - 8 * 8 - 1, 30);
        ...
        osdDrawDecimal2(FONT_16PX_FIXED, sensorData.amps, 5, 0, 2);
        osdDrawCharacter('A', FONT_16PX_FIXED);
        ...
        osdDrawDecimal(FONT_8PX_PROP, multiwiiData.angleRoll, 3, 0, 1);
				...
				osdSetCursor(3, 50 + 16 * 7);
        osdDrawCharacter('I', FONT_16PX_FIXED);
        osdDrawCharacter('d', FONT_16PX_FIXED);
        osdDrawCharacter('l', FONT_16PX_FIXED);
        osdDrawCharacter('e', FONT_16PX_FIXED);
        osdDrawDecimal(FONT_16PX_FIXED, idlePercent, 3, 3, 3);
        CoTickDelay(10);
    }
}
